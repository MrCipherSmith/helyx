...
### FR-2: Remote-сессия
Одна remote-сессия на проект (`source='remote'`, `status='active'|'inactive'`). Создаётся при первом запуске проекта. Не удаляется никогда — только меняет статус. Запускается из Telegram (`/projects` → Start) или из терминала (`helyx start --project <name>`). Подключение = attach к tmux-сессии `bots`.
...
Feature: Persistent Projects
  Scenario: Add new project
    ...
    When пользователь вызывает /project_add helyx /home/user/bots/helyx
...
Feature: Session Switch Briefing
  Scenario: Переключение с существующим саммари
    Given memories содержит project_context для проекта 'helyx'
