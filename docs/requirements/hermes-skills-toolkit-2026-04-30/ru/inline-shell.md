...
- **FR-A-2** — Для каждого совпадения препроцессор ВЫПОЛНЯЕТ захваченную команду через `Bun.spawn(['bash', '-c', cmd])` с `cwd=process.cwd()` (рабочая директория host-side процесса channel.ts), `timeout=5000ms` (настраивается env), `stdout=pipe`, `stderr=pipe`.
...
**Технические**:
- Препроцессор ВЫПОЛНЯЕТСЯ там, где запущен MCP handler `skill_view` — `channel/tools.ts` на хосте И `mcp/tools.ts` внутри Claude Code subprocess. Оба пути делегируют общему `utils/skill-handlers.ts::handleSkillView`, поэтому семантика spawn, env allowlist, timeout и cap идентичны.
- НЕ ДОЛЖЕН выполняться от root: процесс на хосте — от пользователя, запустившего `channel.ts`; в Docker — от пользователя `bun`.
- ДОЛЖЕН использовать `Bun.spawn(['bash', '-c', cmd])` с командой только из SKILL.md (автор скилла доверенный, но всё логируем в `skill_preprocess_log` per FR-A-10)
- ДОЛЖЕН передавать explicit env allowlist (PATH/HOME/LANG/…) в `Bun.spawn`; process.env НЕ наследуется, поэтому `` !`echo $DEEPSEEK_API_KEY` `` не сможет утечь секреты
- ДОЛЖЕН добавить postgres-миграцию (PRD-canonical имя v39_hermes_create_skill_preprocess_log; локальный реестр sequential v23 — схемы совпадают)
...
- `memory/db.ts` — регистрация миграции `v39_hermes_create_skill_preprocess_log`
